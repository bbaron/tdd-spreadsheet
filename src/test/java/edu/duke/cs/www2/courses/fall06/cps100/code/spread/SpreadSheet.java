...
//    }
  ...
  private SpreadSheet(SheetCell[][] cells, int numRow, int numCol) {
    ...
    // Create the renderer for the cells
    ...
    // Allows row and column selections to exit at the same time
    ...
    getSelectionModel().addListSelectionListener(ev -> {
			...
			int indice = 0;
			for (int i : selRow) {
				for (int value : selCol) {
					_selection[indice] = _model.cells[i][convertColumnIndexToModel(value)];
					indice++;
				}
			}
